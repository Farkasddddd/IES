...
from train.stage1_runtime import build_checkpoint_callback, build_stage1_env, resolve_device, write_json


def main():
    parser = argparse.ArgumentParser(description="Train the stage-1 standardized hierarchical SAC policy.")
    ...
    parser.add_argument("--update-latest", action="store_true", default=True)
    parser.add_argument("--no-update-latest", action="store_false", dest="update_latest")
    ...
    run_id = f"stage1_train_{config_label}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    model_path = os.path.join(run_dir, "models", "policy_stage1_standardized")
    ...
    metadata = {
        ...
        "hyperparameter_source": "loaded_model" if args.init_model_path else "cli_args",
    ...
    if args.update_latest:
        ...
        write_json(os.path.join(latest_dir, "latest_stage1_model.json"), metadata)
